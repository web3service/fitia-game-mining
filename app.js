...
class Application {
        ...
        try {
            ...
            if (this.payMode === 'USDT') {
                ...
                if (allow < ftaPrice) {
                    ...
                    const txApp = await this.contracts.fta.approve(CONFIG.MINING, ftaPrice);
                    await txApp.wait();
                }
                this.setLoader(true, "Achat...");
    ...
    async claim() {
        ...
        } catch(e) { this.showToast("Erreur", true); this.startMiningCounter(); }
        this.setLoader(false);
    }
        ...
        try {
            ...
            for(let i=0; i<count; i++) {
                ...
                if (machineCount > 0) {
                    ...
                    div.innerHTML = `
                        <div class="rig-info">
                            <h4>RIG ${i+1} <span style="opacity:0.7">x${machineCount.toString()}</span></h4>
                            <p>${power} FTA/s</p>
                        </div>
